...
-- 2020-08-19,SDawkins,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_SDawkins')
	 Begin 
	  Alter Database [Assignment07DB_SDawkins] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_SDawkins;
	 End
	Create Database Assignment07DB_SDawkins;
End Try
Begin Catch
	Print Error_Number();
End Catch
...
-- Create Function fProducts() 
 ...
 Create -- Drop
 Function dbo.fProducts()
 Returns Table 
 AS 
   Return(
    Select Top 10000000000
	  ProductName, 
	  Format(UnitPrice, 'C', 'en-US') AS 'US Format'
	From dbo.vProducts);
go

Select * from fProducts() Order By ProductName; 
go
...
-- Order the result by the Category and Product!
 ...
 Create -- Drop
 Function dbo.fProductsCategory()
 Returns Table 
 AS 
   Return(
     Select Top 10000000000
...
Select * From fProductsCategory() Order By 1, 2
Go
...
Go
Create -- Drop
 Function dbo.fProductsInventeroyDate()
 Returns Table 
 AS 
   Return
     (Select Distinct Top 10000000000
       P.ProductName, 
	   "InventoryDate" =  DateName(mm, I.Inventorydate) + ', ' +
...
Select * from fProductsInventeroyDate() Order By 1, Cast ([InventoryDate] as Date), 3
Go
...
Select * From vProductInventories Order By 1, Cast([InventoryDate] as Date), 3;
Go
...
Select * From vCategoryInventories Order By 1, Cast([InventoryDate] as Date), 3;
Go
...
Create -- Drop
 View dbo.vProductInventoriesWithPreviouMonthCounts
 AS 
    Select Distinct Top 1000000000
       P.ProductName, 
	   "InventoryDate" = DateName(mm, I.Inventorydate) + ', ' +
	  ...
	  "PreviousMonthCount" = IIF(Year(InventoryDate) = 2016, 0 ,IsNull( Lag(Sum(Count)) Over (Order By ProductName,Year(InventoryDate)), 0))
	 From dbo.vProducts As P
	 Inner Join vInventories As I
	 On P.ProductID =I.ProductID
	 Group By ProductName, InventoryDate, Year(InventoryDate)
Go

Select * From vProductInventoriesWithPreviouMonthCounts Order By 1, Cast([InventoryDate] as Date), 3;
go
...
Create -- Drop
View dbo.vProductInventoriesWithPreviousMonthCountsWithKPIs
As
...
RETURN  
  SELECT TOP 100000000
     ...
     InventoryCount,
     PreviousMonthCount,
     [CountVsPreivousCountKPI] 
  FROM dbo.vProductInventoriesWithPreviousMonthCountsWithKPIs
     Where CountVsPreivousCountKPI = @QtyChangeKPI
  ORDER BY Cast(InventoryDate as Date)
go
